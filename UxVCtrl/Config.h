...
#pragma region Devices parameters
		...
		if (fgets3(file, line, sizeof(line)) == NULL) printf("Error reading parameter : bDisableMS583730BA.\n");
		if (sscanf(line, "%d", &bDisableMS583730BA) != 1) printf("Invalid parameter : bDisableMS583730BA.\n");
...
#pragma region Devices parameters
	...
	if (fgetscopy3(filein, fileout, line, sizeof(line)) == NULL) printf("Invalid parameter : bDisableMS583730BA.\n");
	if (fprintf(fileout, "%d\n", bDisableMS583730BA) < 0) printf("Error writing parameter : bDisableMS583730BA.\n");
...
{
	printf("On OpenCVGUI : \n");
	printf("%c%c%c%c(horizontal control),fvB(vertical control or thrust limit),%c%c(lateral),%c(brake),space(stop),g(generalstop),t%c%cUHT(heading, depth, alt_agl, pitch, roll, obstacle avoidance control),"
